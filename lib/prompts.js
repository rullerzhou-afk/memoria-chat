...
function selectMemoryForPrompt(store, budget = MEMORY_TOKEN_BUDGET) {
  ...
  const sortByScore = (a, b) => computeMemoryScore(b) - computeMemoryScore(a);

  const identity = safe.identity; // 保持原始顺序
  const preferences = [...safe.preferences].sort(sortByScore);
  const events = [...safe.events].sort(sortByScore);
